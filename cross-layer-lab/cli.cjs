...
async function requestSession(token, config) {
  const response = await fetch('http://127.0.0.1:8845/v1/session', { method: 'PUT', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify(config) });
...
async function main() {
  ...
  lab.server.listen(8845, '127.0.0.1', async () => {
    ...
    console.log(`One-time Bundle tab token: ${lab.token}`);
    if (args.brand || args.pageEnv || args.bundleEnv || args.mode) {
      try {
        const session = await requestSession(lab.token, args);
