...
(function () {
  ...
  async function generateLink(opts) {
    ...
    const r=await fetchInternal(base+'/api/customers/'+brandGuid);
		...
		// Logged-out language/currency variants are generated by the same
		// advanced-customer convention used by the internal generate-link UI,
		// so an exact requested key does not have to be pre-listed by
		// /api/customers. It must still belong to the requested login state.
		var customerKey=opts.customerKey||keys[0];
		...
		if(opts.requestedLanguageCode||opts.requestedCurrencyCode) {
			...
			var customer=userNode.loggedOutCustomer||{};
			var actualLanguage=(customer.languageCode||'').toLowerCase();
			var actualCurrency=(((customer.customerWallets||{}).activeWalletCurrency)||'').toUpperCase();
			var wantedLanguage=(opts.requestedLanguageCode||'').toLowerCase();
			var wantedCurrency=(opts.requestedCurrencyCode||'').toUpperCase();
			if((wantedLanguage&&actualLanguage!==wantedLanguage)||(wantedCurrency&&actualCurrency!==wantedCurrency)) {
				throw new Error('Generated context mismatch: requested '+(wantedLanguage||'-')+'/'+(wantedCurrency||'-')+
  ...
  function parseLoggedOutCustomerKey(customerKey) {
    var match = /^(logged-out)-([a-z]{2})-([a-z0-9]{3})-([a-z0-9]+)(.*)$/i.exec(customerKey || '');
    if (!match) return null;
    return {
      languageCode: match[2].toLowerCase(),
      currencyCode: match[3].toUpperCase(),
      jurisdictionCode: match[4],
      suffix: match[5] || ''
    };
  }

  function buildLoggedOutCustomerKey(baseCustomerKey, languageCode, currencyCode) {
    var parsed = parseLoggedOutCustomerKey(baseCustomerKey);
    if (!parsed || !languageCode || !currencyCode) return baseCustomerKey;
    return 'logged-out-' + languageCode.toLowerCase() + '-' + currencyCode.toLowerCase() + '-' +
  ...
  function buildModeA(localLinksPanel) {
    var wrap = el('div', {});
    var brandSel = el('select', { id: 'lgt-gen-brand', name: 'brand' }, brandOptions());
    var envSel = el('select', { id: 'lgt-gen-environment', name: 'environment' }, ENV_LABELS.map(function (e) { return el('option', { value: e }, [e]); }));
    var loginSel = el('select', { id: 'lgt-gen-login-state', name: 'login-state' }, [el('option', { value: 'out' }, ['logged-out']), el('option', { value: 'in' }, ['logged-in'])]);
    ...
    var contextOptionsStatus = el('div', { class: 'lgt-context-status', style: 'display:none' });
    var contextOptionsArea = el('div', { class: 'lgt-context-options' }, [
      customerWrap,
      languageWrap,
      currencyWrap,
      contextOptionsStatus
    ]);
    var contextOptionsToken = 0;
    var contextOptionsLoading = false;
    var currentCustomers = {};
    var currentCustomerKeys = [];
    var currentBrandMetadata = null;
    var brandMetadataCache = {};
    var lastLocaleScope = null;
    // Set once from restored state (see restoreGenState below) and
    // consumed exactly once by the first refreshContextOptions() run
    ...
    var pendingRestoreLanguageCode = null;
    var pendingRestoreCurrencyCode = null;
    ...
    function localeScope() {
      return brandSel.value + '|' + effectiveApiEnvironment();
    }

    function setContextOptionsLoading(loading) {
      contextOptionsLoading = loading;
      contextOptionsArea.setAttribute('aria-busy', loading ? 'true' : 'false');
      if (loading) {
        contextOptionsStatus.className = 'lgt-context-status';
        contextOptionsStatus.textContent = 'Loading context options…';
        contextOptionsStatus.style.display = '';
      }
      [btn, desktopBtn, mobileBtn].forEach(function (button) {
        if (button) button.disabled = loading || generationInProgress;
      });
    }

    function normalizeMetadataOptions(items, codeField) {
      var seen = {};
      return (items || []).reduce(function (result, item) {
        var code = String((item || {})[codeField] || '').trim();
        if (!code || seen[code.toLowerCase()]) return result;
        seen[code.toLowerCase()] = true;
        var name = String((item || {}).name || code).trim();
        // TEST/QA currently expose the Danish locale label as the literal
        // placeholder "Test" for the two DK profiles, while ALPHA/PROD and
        // the real Danish sites identify the same `da` code as Danish.
        if (code.toLowerCase() === 'da' && name.toLowerCase() === 'test') name = 'Danish';
        result.push({ code: code, name: name });
        return result;
      }, []).sort(function (a, b) { return a.name.localeCompare(b.name); });
    }

    async function fetchBrandMetadata(brand, apiEnvironment) {
      var brandGuid = BRANDS[brand];
      var cacheKey = apiEnvironment + '|' + brandGuid;
      if (brandMetadataCache[cacheKey]) return brandMetadataCache[cacheKey];

      async function fetchFrom(environment) {
        var response = await fetchInternal(apiBase(environment) + '/api/brands/' + brandGuid);
        if (!response.ok) throw new Error('brand metadata fetch failed: HTTP ' + response.status);
        var payload = await response.json();
        var data = (payload || {}).data || {};
        return {
          languages: normalizeMetadataOptions(data.supportedLanguages, 'languageCode'),
          currencies: normalizeMetadataOptions(data.supportedCurrencies, 'currencyCode'),
          sourceEnvironment: environment,
          fallback: environment !== apiEnvironment
        };
      }

      try {
        var metadata = await fetchFrom(apiEnvironment);
        brandMetadataCache[cacheKey] = metadata;
        return metadata;
      } catch (error) {
        // Sandbox's metadata route is currently broken on ALPHA/PROD.
        // Its QA registry is the explicit, user-approved fallback; this is
        // always disclosed in the UI rather than silently mixing sources.
        if (brand === 'sandbox' && (apiEnvironment === 'alpha' || apiEnvironment === 'prod')) {
          var fallbackMetadata = await fetchFrom('qa');
          brandMetadataCache[cacheKey] = fallbackMetadata;
    ...
    function selectedBaseCustomerKey() {
      if (customerSelect.value) return customerSelect.value;
      return currentCustomerKeys[0] || '';
    }

    function setSelectOptions(select, options, selectedCode) {
      select.innerHTML = '';
      options.forEach(function (option) {
        var codeLabel = option.code.toUpperCase();
        select.appendChild(el('option', { value: option.code }, [option.name + ' (' + codeLabel + ')']));
      });
      if (selectedCode && options.some(function (option) { return option.code.toLowerCase() === selectedCode.toLowerCase(); })) {
        var exact = options.filter(function (option) { return option.code.toLowerCase() === selectedCode.toLowerCase(); })[0];
        select.value = exact.code;
      }
      if (!select.value && options[0]) select.value = options[0].code;
    }
    ...
    function populateLocaleControls(preserveCurrentSelection) {
      var isLoggedOut = loginSel.value === 'out';
      var baseCustomerKey = selectedBaseCustomerKey();
      var isPassiveCo = baseCustomerKey === BETSSONCO_CO_LIVE_KEY;
      if (!isLoggedOut || !currentBrandMetadata || isPassiveCo) {
        languageWrap.style.display = 'none';
        currencyWrap.style.display = 'none';
        if (!currentBrandMetadata || !isLoggedOut || isPassiveCo) contextOptionsStatus.style.display = 'none';
        return;
      }

      if (!currentBrandMetadata.languages.length || !currentBrandMetadata.currencies.length) {
        languageWrap.style.display = 'none';
        currencyWrap.style.display = 'none';
        contextOptionsStatus.className = 'lgt-context-status error';
        contextOptionsStatus.textContent = '⚠ Context metadata is incomplete — the default customer context will be used.';
        contextOptionsStatus.style.display = '';
        currentBrandMetadata = null;
        return;
      }

      var parsedBase = parseLoggedOutCustomerKey(baseCustomerKey) || {};
      var scope = localeScope();
      var restoredForThisScope = pendingRestoreLocaleScope === scope;
      var languageCandidate = restoredForThisScope ? pendingRestoreLanguageCode :
        (preserveCurrentSelection && lastLocaleScope === scope ? languageSelect.value : parsedBase.languageCode);
      var currencyCandidate = restoredForThisScope ? pendingRestoreCurrencyCode :
        (preserveCurrentSelection && lastLocaleScope === scope ? currencySelect.value : parsedBase.currencyCode);

      setSelectOptions(languageSelect, currentBrandMetadata.languages, languageCandidate);
      setSelectOptions(currencySelect, currentBrandMetadata.currencies, currencyCandidate);
      languageWrap.style.display = currentBrandMetadata.languages.length > 1 ? '' : 'none';
      currencyWrap.style.display = currentBrandMetadata.currencies.length > 1 ? '' : 'none';
      ...
      if (currentBrandMetadata.fallback) {
        ...
        contextOptionsStatus.style.display = 'none';
      }

      pendingRestoreLanguageCode = null;
      pendingRestoreCurrencyCode = null;
      pendingRestoreLocaleScope = null;
      lastLocaleScope = scope;
      saveLocaleState();
    }

    function populateCustomerOptions(customers, prefix) {
      currentCustomers = customers || {};
      currentCustomerKeys = Object.keys(currentCustomers).filter(function (key) {
        return key.toLowerCase().indexOf(prefix) === 0;
      });
      var syntheticKeys = (brandSel.value === 'betssonco' && prefix === 'logged-out')
        ? [{ value: BETSSONCO_CO_LIVE_KEY, label: 'CO - Logged Out (live capture from betsson.co)' }]
        : [];

      customerSelect.innerHTML = '';
      currentCustomerKeys.forEach(function (key) {
        customerSelect.appendChild(el('option', { value: key }, [(currentCustomers[key] || {}).label || key]));
      });
      syntheticKeys.forEach(function (entry) {
        customerSelect.appendChild(el('option', { value: entry.value }, [entry.label]));
      });
      if (pendingRestoreCustomerKey && (currentCustomerKeys.indexOf(pendingRestoreCustomerKey) !== -1 ||
        syntheticKeys.some(function (entry) { return entry.value === pendingRestoreCustomerKey; }))) {
        customerSelect.value = pendingRestoreCustomerKey;
      }
      pendingRestoreCustomerKey = null;
      customerWrap.style.display = currentCustomerKeys.length + syntheticKeys.length > 1 ? '' : 'none';
    }

    function refreshContextOptions() {
      var brand = brandSel.value;
      var brandGuid = BRANDS[brand];
      var token = ++contextOptionsToken;
      if (!brandGuid) { contextOptionsArea.style.display = 'none'; return; }
      contextOptionsArea.style.display = '';
      var apiEnv = effectiveApiEnvironment();
      var prefix = loginSel.value === 'in' ? 'logged-in' : 'logged-out';
      setContextOptionsLoading(true);
      (async () => {
        try {
          async function fetchMetadataResult() {
            try { return await fetchBrandMetadata(brand, apiEnv); }
            catch (error) { return { error: error }; }
          }
          var customerRequest = fetchInternal(apiBase(apiEnv) + '/api/customers/' + brandGuid);
          var metadataRequest = prefix === 'logged-out' ? fetchMetadataResult() : null;
          var customerResponse = await customerRequest;
          var metadataResult = metadataRequest ? await metadataRequest : null;
          if (!customerResponse.ok) throw new Error('customers fetch failed: HTTP ' + customerResponse.status);
          var customers = await customerResponse.json();
          if (token !== contextOptionsToken) return;

          populateCustomerOptions(customers, prefix);
          currentBrandMetadata = metadataResult && !metadataResult.error ? metadataResult : null;
          if (metadataResult && metadataResult.error) {
            languageWrap.style.display = 'none';
            currencyWrap.style.display = 'none';
            contextOptionsStatus.className = 'lgt-context-status error';
            contextOptionsStatus.textContent = '⚠ Context options unavailable — the default customer context will be used.';
            contextOptionsStatus.style.display = '';
          } else {
            populateLocaleControls(false);
          }
        } catch (error) {
          if (token !== contextOptionsToken) return;
          currentCustomers = {};
          ...
          currencyWrap.style.display = 'none';
          contextOptionsStatus.className = 'lgt-context-status error';
          contextOptionsStatus.textContent = '⚠ Customer options unavailable: ' + friendlyErrorMessage(error);
          contextOptionsStatus.style.display = '';
        } finally {
          if (token === contextOptionsToken) setContextOptionsLoading(false);
        }
      })();
    }
    ...
    customerSelect.addEventListener('change', function () {
      saveGenState({ customerKey: customerSelect.value });
      populateLocaleControls(true);
    });
    languageSelect.addEventListener('change', saveLocaleState);
    currencySelect.addEventListener('change', saveLocaleState);
    ...
    chrome.storage.local.get([GEN_STATE_KEY], function (res) {
      ...
      if (saved) {
        ...
        if (saved.languageCode) pendingRestoreLanguageCode = saved.languageCode;
        if (saved.currencyCode) pendingRestoreCurrencyCode = saved.currencyCode;
        if (saved.localeScope) pendingRestoreLocaleScope = saved.localeScope;
      }
    ...
    function selectedGeneratedCustomerKey() {
      var baseCustomerKey = selectedBaseCustomerKey();
      if (!baseCustomerKey || baseCustomerKey === BETSSONCO_CO_LIVE_KEY || !currentBrandMetadata) return baseCustomerKey;
      return buildLoggedOutCustomerKey(baseCustomerKey, languageSelect.value, currencySelect.value);
    }

    function selectedGeneratedCustomerLabel() {
      if (!currentBrandMetadata) return null;
      var language = currentBrandMetadata.languages.filter(function (item) { return item.code === languageSelect.value; })[0];
      var currency = currentBrandMetadata.currencies.filter(function (item) { return item.code === currencySelect.value; })[0];
      if (!language || !currency) return null;
      return language.name + ' (' + language.code.toUpperCase() + ') · ' +
        currency.name + ' (' + currency.code.toUpperCase() + ') · Logged Out';
    }
    ...
    function runGenerateFlow(devices, clickedBtn, clickedLabel) {
      ...
      return function attempt() {
        ...
        ensureFreshResultFor(resultKeyFor(brand, environment, loggedIn, bleSource, generatedCustomerKey));
        log.textContent = 'Generating...';

        function setBtnBusy(busy) {
          generationInProgress = busy;
          [btn, desktopBtn, mobileBtn].forEach(function (b) { if (b) b.disabled = busy || contextOptionsLoading; });
...
const links = await generateLink({ brand: brand, environment: environment, loggedIn: false, customerKeyFilter: '', bleSource: bleSourceWanted });
        ...
        if (!loggedIn) {
          ...
          // fallback below).
          if (baseCustomerKey === BETSSONCO_CO_LIVE_KEY) {
          ...
          (async () => {
try {
return renderLinks(await generateLink({
  brand: brand,
  environment: environment,
  loggedIn: false,
  customerKey: generatedCustomerKey,
  requestedLanguageCode: currentBrandMetadata ? languageSelect.value : '',
  requestedCurrencyCode: currentBrandMetadata ? currencySelect.value : '',
  customerLabel: selectedGeneratedCustomerLabel(),
  bleSource: bleSource
}));
} catch (err) {
...
const hasKey = await hasLoggedInCustomerKey(brand, bleSource ? 'prod' : environment);
          ...
          if (hasKey) {
            (async () => {
try {
return renderLinks(await generateLink({ brand: brand, environment: environment, loggedIn: true, customerKeyFilter: baseCustomerKey, bleSource: bleSource }));
} catch (err) {
                log.textContent = 'Error: ' + friendlyErrorMessage(err);
                setBtnBusy(false);
                if (err && err.isVpnRequired) showVpnRequiredPopup(err.message, attempt);
...
const hasKey = await hasLoggedInCustomerKey(brand, bleSource ? 'prod' : environment);
    ...
    wrap.appendChild(el('label', { for: 'lgt-gen-brand' }, ['Brand']));
    wrap.appendChild(brandSel);
    wrap.appendChild(el('div', { class: 'lgt-row' }, [
      (function () { var d = el('div', {}); d.appendChild(el('label', { for: 'lgt-gen-environment' }, ['Environment'])); d.appendChild(envSel); return d; })(),
